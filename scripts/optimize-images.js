...
function classifyImage(filename) {
  ...
  if (name.includes('nacional') || name.includes('almacen') || 
      ...
      name.includes('sede') || name.includes('oficina')) {
    return 'service';
  }
...
async function main() {
  ...
  console.log(`📁 Encontradas ${images.length} imágenes PNG\n`);
  ...
  for (const imagePath of images) {
    ...
    if (doOptimize || doAll) {
      ...
      const result = await optimizePNG(imagePath, optimizedPath);
      if (result.success) {
        totalOptimizedSize += result.optimizedSize;
        console.log(`   ✅ PNG optimizado: ${(result.optimizedSize / 1024 / 1024).toFixed(2)} MB (reducción: ${result.reduction})`);
        results.push({
          file: relativePath,
          type: 'PNG optimized',
